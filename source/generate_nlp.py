...
def generate_cot_dataset(seed_start=1, seed_end=100, size=5, output_dir="data"):
    ...
    sx, sy = actual_start
    output_name = f"cot_{size}x{size}_{seed_start}to{seed_end}_{sx}{sy}nlp.jsonl"
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for seed in range(seed_start, seed_end + 1):
            ...
            input_text = f"Start={s[0]},{s[1]}\nActions: " + ", ".join(words)
            output_text = generate_cot_steps(s, actions)
...
if __name__ == "__main__":
    ...
    print(f"✅ NLP-style dataset saved to: {path}")
